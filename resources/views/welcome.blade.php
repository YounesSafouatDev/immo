...
@section('content')
    ...
    <section class="banner">
        ...
        <a href="{{ route('biens') }}" class="banner_link">PREMIUM IMMOBILIERS &nbsp;&nbsp;<i
                class="fa-solid fa-magnifying-glass" style="font-size: 18px;"></i></a>
    </section>
    <!-- Main -->
    <main class="main">
        <div class="text">
        ...
        <section class="list_categories">
            @foreach ($categories as $category)
                <div class="category">
                    <img src="{{ asset('assets/images/' . $category->name . '.png') }}" alt="image immobilier"
                        class="category_image" loading="lazy">
        ...
        <section class="properties_cards">
            @if ($annonces->isNotEmpty())
                @foreach ($annonces as $annonce)
                    <div class="property">
                        <div class="cover"
                            style="background-image: url('{{ $annonce->images->isNotEmpty() ? asset('storage/annonces/' . $annonce->images->first()->path) : asset('images/default_image.jpg') }}');">
                            <h3>{{ Str::limit($annonce->title, 140) }}...</h3>
                            <span class="price">{{ $annonce->price }} Dhs</span>

                            <div class="card-back">
                                <a href="{{ route('bien', $annonce->id) }}" class="back_link">Plus de détails</a>
                            </div>
                        </div>
                    </div>
                @endforeach
            ...
            @if ($annonceurs->isNotEmpty())
                @foreach ($annonceurs as $annonceur)
                    <div class="card card0" style="background-image:url('{{ asset('assets/images/agent.webp') }}')">
                        <div class="border">
                            <a href="{{ route('biens') }}" class="agent_link">{{ $annonceur->fname }}
                                {{ $annonceur->lname }}</a>
                            <div class="icons">
                                <a href="https://wa.me/+212.{{ $annonceur->whatsapp }}"><i class="fa fa-whatsapp"
                                        aria-hidden="true"></i></a>
                                <a href="{{ $annonceur->instagram }}"><i class="fa fa-instagram"
                                        aria-hidden="true"></i></a>
                ...
                @foreach ($articles as $article)
                    <div class = "article">
                        @if ($article->images->isNotEmpty())
                            <img src="{{ asset('storage/articles/' . $article->images->last()->path) }}"
