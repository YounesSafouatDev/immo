...
@section('main')
    <main>
        ...
        <div class="new_article">
            <form action="{{route('annonces.store')}}" method="post" class="form" enctype="multipart/form-data">
                @csrf
                ...
                <div class="input_group_select">
                    ...
                    <select name="annonceur" required>
                        <option value="0" selected disabled>Choisir Un Annonceur</option>
                        @foreach ($utilisateurs as $utilisateur)
                            <option value="{{$utilisateur->id}}">{{$utilisateur->fname}} {{$utilisateur->lname}}</option>
                        @endforeach
                    </select>
                    <span class="error">{{ $errors->first('annonceur') }}</span>
                </div>
                <div class="input_group_select">
                    <label for="category">Catégorie Du Bien <span class="required">*</span></label>
                    <select name="category" required>
                        <option value="0" selected disabled>Choisir Catégorie</option>
