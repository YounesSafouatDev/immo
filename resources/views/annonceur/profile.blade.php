...
@section('main')
    <main>
        <h1 class="title">Modifier Annonceur</h1>
        <ul class="breadcrumbs">
            <li><a href="{{route('annonceur.dashboard')}}">Acceuil</a></li>
            <li class="divider">/</li>
            <li><a href="#" class="active">Modifier</a></li>
            <li class="divider">/</li>
            <li><a href="#" class="active">{{$annonceur->id}}</a></li>
        </ul>
        <h2 class="title_infos">Information Personnelle</h2>
        <section class="ajouter_utilis">
            <form action="{{route('annon.update',$annonceur->id)}}" method="POST" class="utilis_form">
                @csrf
                <div class="input_group">
                    <label for="lname">Nom <span class="required">*</span></label>
                    <input type="text" name="lname" class="input" id="nom" required placeholder="Nom" value="{{$annonceur->lname}}">
                    <span class="message" id="message_last">{{ $errors->first('lname') }}</span>
                </div>
                <div class="input_group">
                    <label for="lname">Prénom <span class="required">*</span></label>
                    <input type="text" name="fname" class="input" id="prenom" required placeholder="Prénom" value="{{$annonceur->fname}}">
                    <span class="message" id="message_first">{{ $errors->first('fname') }}</span>
                </div>
                <div class="input_group">
                    <label for="email">Email <span class="required">*</span></label>
                    <input type="email" name="email" class="input" id="email" required placeholder="Votre E-mail Adresse" value="{{$annonceur->email}}">
                    <span class="message" id="message_email">{{ $errors->first('email') }}</span>
                </div>
                <div class="input_group">
                    <label for="lname">Téléphone <span class="required">*</span></label>
                    <input type="text" name="phone" class="input" id="phone" required minlength="10" maxlength="10" placeholder="Votre Téléphone" value="{{$annonceur->phone}}">
            ...
            <form action="{{route('annonceur.reseau',$annonceur->id)}}" method="POST" class="url_form">
                @csrf
                <div class="input_group">
                    <label for="whatsapp">Whatsapp</label>
                    <input type="text" name="whatsapp" class="input" id="whatsapp" minlength="10" maxlength="10" placeholder="Numéro Téléphone Whatsapp" value="{{$annonceur->whatsapp}}">
                    <span class="message">{{ $errors->first('whatsapp') }}</span>
                </div>
                <div class="input_group">
                    <label for="facebook">Facebook</label>
                    <input type="text" name="facebook" class="input" id="facebook" placeholder="Votre Profil Facebook" value="{{$annonceur->facebook}}">
                </div>
                <div class="input_group">
                    <label for="instagram">Instagram</label>
                    <input type="text" name="instagram" class="input" id="instagram" placeholder="Votre Profil Instagram" value="{{$annonceur->instagram}}">
                </div>
                <button class="btn">Ajouter</button>
            </form> 
        <h2 class="title_infos">Mot De Passe</h2>
        <form action="{{route('annonceur.changermdp',$annonceur->id)}}" method="POST" class="url_form">
            @csrf
            <div class="input_group">
